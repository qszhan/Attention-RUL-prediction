...
class LSTM1(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, seq_length=1, device='cpu'):
    ...
    def forward(self, x):
        ...
        hn_o = torch.Tensor(hn.cpu().detach().numpy()[-1, :, :])
        hn_o = hn_o.view(-1, self.hidden_size)
        hn_1 = torch.Tensor(hn.cpu().detach().numpy()[1, :, :])
