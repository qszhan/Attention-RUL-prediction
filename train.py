...
class Trainer:
    ...
    def train_single_epoch(self, dataloader):
        ...
        for batch_index, data in enumerate(dataloader, 0):
            ...
            inputs = inputs.float()
            handcrafted_feature = handcrafted_feature.float()
            labels = labels.float()
    ...
    def train(self, train_loader, test_loader, iteration, corr_indicator=False):
        for epoch in range(self.epochs):
            ...
            if epoch == 0:
                ...
                if current_RMSE < best_RMSE:
                    best_RMSE = current_RMSE
                    if corr_indicator == False:
                        self.save_checkpoints(iteration + 1, epoch + 1, 'best_RMSE')
                    else:
                        self.save_checkpoints(iteration + 1, epoch + 1, 'empi_RMSE_tl')
    ...
    def save_checkpoints(self, iteration, epoch, which_type):
        ...
        torch.save(state, './checkpoints/{}_{}.pth.tar'.format(self.prefix, which_type))
