...
class Turbofandataset(Dataset):
    def __init__(self, mode='train', dataset=None, rul_result=None):
        ...
        self.data= self.data[:, ~np.isnan(self.data).all(axis=0)]
        self.window_size = 30
        if dataset == './datasets/CMAPSSData/test_FD002_normed.txt' or './datasets/CMAPSSData/train_FD002qs_normed.txt':
            self.sample_num = int(self.data[-1][0]) - 1
        self.length = []
        self.mode = mode
